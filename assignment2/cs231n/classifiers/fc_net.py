...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    N=X.shape[0]
    D=np.prod(X.shape[1:])
    inputs=X.reshape(N,D)

    h1,cache1 = affine_relu_forward(inputs,self.params['W1'],self.params['b1'])
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1,self.num_layers+1):
        if (i==1):
            self.params['W'+repr(i)]=np.random.normal(size=(input_dim,hidden_dims[i-1]),scale=weight_scale)
  ...
  def loss(self, X, y=None):
    ...
    N=X.shape[0]
    D=np.prod(X.shape[1:])
    forward_inputs=X.reshape(N,D)
    cache={}
    for i in range(1,self.num_layers+1):
        if (self.use_dropout and i<self.num_layers):
            forward_inputs,cache['dropout'+repr(i)]=dropout_forward(forward_inputs,self.dropout_param)
    ...
    for i in range(self.num_layers,0,-1):
        ...
        if (self.use_dropout and i<self.num_layers):
            dout=dropout_backward(dout,cache['dropout'+repr(i)])
